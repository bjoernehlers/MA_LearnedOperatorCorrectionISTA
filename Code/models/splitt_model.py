...
class My_split_UNet(nn.Module):
    ...
    def forward(self,x):
        ...
        for rt_layer in self.ray_trafo_layers:
            x_j = x
            for i in range(self.num_of_downs):
                x_j = self.ups[j*self.num_of_downs+i](l[-(i+1)],x_j)
            x_j = self.outs[j](x_j)
            y.append(rt_layer(x_j))
            j = j+1
